...
#define TOOL_PAN 6
	...
	while (!done)
	{
		...
		if ((tool == TOOL_PAINT || tool == TOOL_PAINT_SQUARE) && !hovered && (ImGui::IsMouseDown(ImGuiMouseButton_Left)) && left_strings.size() > 0)
		{
			...
			float relative_brush_size =  max_brush_size / zoom_factor / tiles_per_chunk_dimension;// 100;////(max_brush_size ) / (tiles_per_chunk_dimension);//// * tiles_per_chunk_dimension;// / block_size;// (float)max_brush_size / (float)block_size *  * zoom_factor;//* tiles_per_chunk_dimension;// *tiles_per_chunk_dimension;
